...
def get_zarr_reader(url):
    logger.debug("Getting image reader for: %s" % url)
    if url not in READER_CACHE:
        # CPA should only use paths
        rdr = ZarrReader(None, url)
        READER_CACHE[url] = rdr
    return READER_CACHE[url]
